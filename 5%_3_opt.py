...
data = pd.read_csv(
    '/Users/AymenHafeez/Desktop/DESKTOP/Research_project/Microalgae_Models/data_2.5.csv')

t1 = data['Day'].values
X1meas = data['dry_weight3'].values
ns = len(t1)
...
def simulate(p):
    ...
    for i in range(len(t1) - 1):
        ts = [t1[i], t1[i + 1]]
        x = odeint(model, X0, ts, args=(p, Tf))
        X0 = x[-1]
        X[i + 1] = X0
    return X
...
plt.plot(t1, X1meas, '.', markersize=12, markerfacecolor=None,
         markeredgecolor='b', markeredgewidth=1, label='Experimental data')
plt.plot(t1, xi[:, 0], 'g--', linewidth=2, label='Initial prediction')
plt.plot(t1, xi[:, 1], 'r-', linewidth=2, label='Optimized predicition')
